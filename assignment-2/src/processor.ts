...
export async function main(args: string[]): Promise<void> {
  try {
    ...
    // 判斷是單一檔案還是批次處理
    const inputStats = await fsPromises
      .stat(parsedArgs.input)
      .catch((error) => {
        if ((error as any).code === "ENOENT") {
          throw new Error(`❌ Input path not found: ${parsedArgs.input}`);
        } else if ((error as any).code === "EACCES") {
          throw new Error(
            `❌ Permission denied accessing: ${parsedArgs.input}`
          );
        } else {
    ...
    if (inputStats.isFile()) {
      ...
      throw new Error(
        `❌ Input path is neither a file nor a directory: ${parsedArgs.input}`
      );
    }
    ...
    if (error instanceof Error) {
      if (error.message.includes("not found")) {
        console.error(
          "💡 Suggestion: Please check if the input file or directory exists"
        );
        ...
        console.error(
          "💡 Suggestion: Use --input=<path> --output=<path> [--format=json|text]"
        );
      }
    }
...
export function parseCommandLineArgs(args: string[]): CommandLineArgs {
  ...
  if (relevantArgs.length === 0) {
    throw new Error(
      "❌ Missing required arguments. Usage: --input=<path> --output=<path> [--format=json|text]"
    );
  }
  ...
  for (const arg of relevantArgs) {
    if (arg.startsWith("--input=")) {
      ...
      if (format === "json" || format === "text") {
        ...
        throw new Error(
          `❌ Invalid format: ${format}. Supported formats: json, text`
        );
      }
...
async function processSingleFile(
  ...
  try {
    ...
    console.log(
      `📋 Successfully read and validated: ${billInput.location} (${billInput.items.length} items)`
    );

    // 使用核心函數處理分帳
    const billOutput = splitBill(billInput);
    console.log(
      `💰 Bill calculation completed for ${billOutput.items.length} people`
    );

    // 寫入輸出檔案
    await writeOutputFile(outputPath, billOutput, format);
    console.log(
      `✅ Successfully processed ${path.basename(inputPath)} -> ${path.basename(
        outputPath
      )}`
    );
  } catch (error) {
...
async function processBatchFiles(
  ...
  try {
    ...
    try {
      ...
      if (err.code === "EACCES") {
        throw new Error(
          `❌ Permission denied creating output directory: ${outputDir}\n💡 Check directory permissions or run with appropriate privileges`
        );
      }
      throw mkdirError;
    }
    ...
    try {
      ...
      if (err.code === "EACCES") {
        throw new Error(
          `❌ Permission denied reading directory: ${inputDir}\n💡 Check directory permissions`
        );
      }
    ...
    const results = await Promise.allSettled(
      jsonFiles.map(async (file) => {
        ...
        const outputFileName =
          path.basename(file, ".json") + (format === "json" ? ".json" : ".txt");
    ...
    if (failed > 0) {
      throw new Error(
        `❌ Batch processing completed with ${failed} failed files out of ${jsonFiles.length} total`
      );
    }
 ...
 * @returns 驗證後的帳單輸入資料
 */
export async function readAndValidateJsonFile(
  filePath: string
): Promise<BillInput> {
  try {
    // 檢查檔案是否存在
...
export async function writeOutputFile(
  ...
  try {
    ...
    try {
      ...
      if (err.code === "EACCES") {
        throw new Error(
          `❌ Permission denied creating directory: ${outputDir}\n💡 Check directory permissions or run with appropriate privileges`
        );
      }
      throw mkdirError;
    }
    ...
    if (err.code === "EACCES") {
      ...
      throw new Error(
        `❌ No space left on device: ${outputPath}\n💡 Free up disk space and try again`
      );
    } else if (err.code === "EISDIR") {
      throw new Error(
        `❌ Output path is a directory, not a file: ${outputPath}\n💡 Specify a file path, not a directory`
      );
    }
